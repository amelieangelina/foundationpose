...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mesh_file",
        type=str,
        default=f"{code_dir}/demo_data/metal_sheet/mesh/metal_sheet_cad.stl",
    )
    parser.add_argument(
    ...
    mesh.apply_scale(0.000025)
    mesh.export(
        f"{code_dir}/demo_data/metal_sheet/mesh/metal_sheet_cad_downsized_more.stl"
    )
    ...
    reader = YcbineoatReader(
        video_dir=args.test_scene_dir, downscale=0.5, shorter_side=None, zfar=np.inf
    )
    ...
    for i in range(len(reader.color_files)):
        ...
        # other results are helping to refine the result of the pose with tracking.
        # The first picture needs a mask first.
        if i == 0:
