...
class YcbineoatReader:
    ...
    def get_depth(self, i):
        path = self.color_files[i].replace("rgb", "depth")
        print("Getting depth from path: " + path)
        # Reading a grayscale image with flag -1 = unchanged
        # The type of the original images is float64
        depth = cv2.imread(path, -1) / 1e3
        if len(depth.shape) == 3:
            # Loading the image with the grayscale flag to load only a 2 dimensional array
            depth = cv2.imread(path, 0) / 1e3
            # The type of the grayscale image is uint8 with the zivid
        print("Image at point 100,100 = " + str(depth[400, 400]))
        maximum_value = 0.0
        ...
        print("Image at point 100,100 = " + str(depth[400, 400]))
        cv2.imwrite(path.replace("1", "depth_check"), depth)
