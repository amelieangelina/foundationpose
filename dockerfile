...
# Note: /foundationpose will only exist if using docker compose as it mounts the local directory to the container
...
# Clean and install Python package in /kaolin
RUN cd /kaolin && \
    rm -rf build *egg* && \
    pip install -e .

# Clean and install Python package in mycuda directory of /foundationpose/bundlesdf
RUN cd /foundationpose/bundlesdf/mycuda && \
    rm -rf build *egg* && \
    pip install -e .

# force system to use bash
ENV SHELL=/bin/bash
